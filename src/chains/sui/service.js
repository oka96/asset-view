...
export class SuiService {
    ...
    async listAssets(address) {
        if (!address) {
            return [];
        }
        const balances = await this.rpcCall('suix_getAllBalances', [address]);
        const assets = await Promise.all(balances.map(async (balance) => {
    ...
    async getNetworkStatus() {
        ...
        try {
            const identifier = await this.rpcCall('sui_getChainIdentifier', []);
    ...
    async fetchMetadata(coinType) {
        if (this.metadataCache.has(coinType)) {
            return this.metadataCache.get(coinType);
        }
        const result = await this.rpcCall('suix_getCoinMetadata', [coinType]);
        const metadata = {
